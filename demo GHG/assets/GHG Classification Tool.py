...
<style>
    ...
    :root {
        --primary-sage: #70798c;
        --primary-orange: #FF4F0D;
        --secondary-orange: #E64408;
        --light-orange: #FFF4F0;
        --light-gray: #EEF0EB;
        --accent-gradient: linear-gradient(135deg, #FF4F0D 0%, #E64408 50%, #FF4F0D 100%);
        --subtle-gradient: linear-gradient(135deg, #E64408 0%, #FF4F0D 35%, #B4B8AB 100%);
        --light-gradient: linear-gradient(135deg, #FFF4F0 0%, #EEF0EB 100%);
        --orange-gradient: linear-gradient(135deg, #FF4F0D 0%, #E64408 50%, #FF4F0D 100%);
    ...
    .main-header {
        ...
        position: relative;
        overflow: hidden;
        box-shadow: 0 8px 32px rgba(255, 79, 13, 0.15);
        min-height: 150px;
        display: flex;
    ...
    .stProgress > div > div > div > div {
        background: linear-gradient(90deg, var(--primary-orange) 0%, #16a34a 100%);
        border-radius: 10px;
    ...
    .metric-card:hover {
        transform: translateY(-4px);
        box-shadow: 0 8px 25px rgba(255, 79, 13, 0.15);
        border-color: #cbd5e1;
    }
...
if st.session_state.file_processed:
    ...
    # Display the line items table with highlighting for duplicates
    ...
    if 'Line Item' in df_display.columns:
        ...
        # If no Line Item column, display normally
...
if st.session_state.file_processed:
    ...
    if st.session_state.pipeline_running:
        ...
        for i, workflow in enumerate(st.session_state.selected_workflows):
            ...
            # # <div style="text-align: center; color: #000000; font-weight: 600; margin: 1rem 0;">
...
if st.session_state.pipeline_complete:
    ...
    if st.session_state.pipeline_complete:
        ...
        st.markdown(f"""
        <div class="metrics-grid">
            <div class="metric-card" style="border-top: 4px solid #FF4F0D;">
            ...
            <div class="metric-card" style="border-top: 4px solid #10b981;">
                ...
                <div class="metric-label">All Classified</div>
            </div>
        ...
        if 'result_1_data' in results:
            ...
            with tab1:
                ...
                if not results['result_1_data'].empty:
                    # Apply styling to highlight NAICS Name column header
                    def highlight_naics_header(df):
                        styles = pd.DataFrame('', index=df.index, columns=df.columns)
                        return styles
                    
                    if 'NAICS Name' in results['result_1_data'].columns:
                        styled_df = results['result_1_data'].style.set_table_styles([
                            {'selector': 'th:contains("NAICS Name")', 
                             'props': [('background-color', '#FF4F0D'), 
                                      ('color', 'black'), 
                                      ('font-weight', 'bold')]}
                        ])
                        st.dataframe(styled_df, use_container_width=True, hide_index=True)
                    else:
                        st.dataframe(results['result_1_data'], use_container_width=True, hide_index=True)
                else:
            ...
            with tab2:
                ...
                if not results['result_2_data'].empty:
                    ...
                    if 'NAICS Name' in results['result_2_data'].columns:
                        styled_df = results['result_2_data'].style.set_table_styles([
                            {'selector': 'th:contains("NAICS Name")', 
                             'props': [('background-color', '#FF4F0D'), 
                                      ('color', 'black'), 
                                      ('font-weight', 'bold')]}
                        ...
                        st.dataframe(results['result_2_data'], use_container_width=True, hide_index=True)
                else:
                    st.info("No data in this tab")
            ...
            with tab3:
                ...
                if not results['result_3_data'].empty:
                    ...
                    if 'NAICS Name' in results['result_3_data'].columns:
                        styled_df = results['result_3_data'].style.set_table_styles([
                            {'selector': 'th:contains("NAICS Name")', 
                             'props': [('background-color', '#FF4F0D'), 
                                      ('color', 'black'), 
                                      ('font-weight', 'bold')]}
                        ...
                        st.dataframe(results['result_3_data'], use_container_width=True, hide_index=True)
                else:
                    st.info("No data in this tab")
