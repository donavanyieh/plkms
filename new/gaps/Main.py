...
def get_mock_materiality_data():
    ...
    return [
        ...
        },
        {
            ...
            "reasoning": "Comprehensive disclosure of climate-related risks and opportunities, including physical and transition risks, scenario analysis, and climate strategy with science-based targets.",
            "citation": "Pages 35-42, Climate Risk Assessment, Table 6.1",
            "ifrs_requirement": "IFRS S2"
        }
    ]
...
if st.session_state.materiality_extracted:
    ...
    if st.session_state.gap_analysis_complete:
        ...
        total_partial = len(df[df['Status'] == 'Partial'])
        total_missing = len(df[df['Status'] == 'Missing'])
        
        # IFRS S1 coverage calculations
        s1_df = df[df['IFRS Requirement'].str.startswith('IFRS S1')]
        s1_total = len(s1_df)
        s1_complete = len(s1_df[s1_df['Status'] == 'Complete'])
        s1_partial = len(s1_df[s1_df['Status'] == 'Partial'])
        s1_missing = len(s1_df[s1_df['Status'] == 'Missing'])
        
        # IFRS S2 coverage calculations
        s2_df = df[df['IFRS Requirement'].str.startswith('IFRS S2')]
        s2_total = len(s2_df)
        s2_complete = len(s2_df[s2_df['Status'] == 'Complete'])
        s2_partial = len(s2_df[s2_df['Status'] == 'Partial'])
        s2_missing = len(s2_df[s2_df['Status'] == 'Missing'])
        
        # Enhanced metrics display
        st.markdown("### ➤ Total Coverage")
        st.markdown('<div style="margin-bottom: 0.75rem;"><span class="status-badge priority-medium" style="font-size: 0.75rem;">Effort to close: Medium</span></div>', unsafe_allow_html=True)
        st.markdown(f"""
        <div class="metrics-grid">
            <div class="metric-card" style="border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                <div class="metric-value">55%</div>
                <div class="metric-label">Coverage Score</div>
            </div>
            <div class="metric-card" style="border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                <div class="metric-value">{total_complete}</div>
                <div class="metric-label">{total_complete/total_requirements*100:.0f}% Fully Covered</div>
            </div>
            <div class="metric-card" style="border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                <div class="metric-value">{total_partial}</div>
                <div class="metric-label">{total_partial/total_requirements*100:.0f}% Partially Covered</div>
            </div>
            <div class="metric-card" style="border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                <div class="metric-value">{total_missing}</div>
                <div class="metric-label">{total_missing/total_requirements*100:.0f}% Not Covered</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        
        # IFRS S1 and S2 Coverage side by side
        col1, col2 = st.columns(2)
        ...
        with col1:
            if s1_total > 0:
                ...
                <div class="content-section">
                    <h4 style="margin-top: 0; color: var(--primary-navy);">IFRS S1 Coverage</h4>
                    <div style="margin-bottom: 1rem;">
                        <span class="status-badge priority-medium" style="font-size: 0.75rem;">Effort to close: Medium</span>
                    </div>
                    <div class="metrics-grid">
                        <div class="metric-card" style="padding: 1.5rem 1rem; text-align: center; border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                            <div class="metric-value" style="font-size: 1.8rem; margin: 0.5rem 0;">55%</div>
                            <div class="metric-label" style="font-size: 0.8rem;">Coverage Score</div>
                        </div>
                        <div class="metric-card" style="padding: 1.5rem 1rem; text-align: center; border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                            <div class="metric-value" style="font-size: 1.8rem; margin: 0.5rem 0;">{s1_complete}</div>
                            <div class="metric-label" style="font-size: 0.8rem;">{s1_complete/s1_total*100:.0f}% Fully Covered</div>
                        </div>
                    </div>
                    <div class="metrics-grid" style="margin-top: 0.5rem;">
                        <div class="metric-card" style="padding: 1.5rem 1rem; text-align: center; border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                            <div class="metric-value" style="font-size: 1.8rem; margin: 0.5rem 0;">{s1_partial}</div>
                            <div class="metric-label" style="font-size: 0.8rem;">{s1_partial/s1_total*100:.0f}% Partially Covered</div>
                        </div>
                        <div class="metric-card" style="padding: 1.5rem 1rem; text-align: center; border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                            <div class="metric-value" style="font-size: 1.8rem; margin: 0.5rem 0;">{s1_missing}</div>
                            <div class="metric-label" style="font-size: 0.8rem;">{s1_missing/s1_total*100:.0f}% Not Covered</div>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                """, unsafe_allow_html=True)
                st.info("No IFRS S1 requirements in current analysis")
        
        with col2:
            if s2_total > 0:
                st.markdown(f"""
                <div class="content-section">
                    <h4 style="margin-top: 0; color: var(--primary-navy);">IFRS S2 Coverage</h4>
                    <div style="margin-bottom: 1rem;">
                        <span class="status-badge priority-medium" style="font-size: 0.75rem;">Effort to close: Medium</span>
                    </div>
                    <div class="metrics-grid">
                        <div class="metric-card" style="padding: 1.5rem 1rem; text-align: center; border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                            <div class="metric-value" style="font-size: 1.8rem; margin: 0.5rem 0;">56%</div>
                            <div class="metric-label" style="font-size: 0.8rem;">Coverage Score</div>
                        </div>
                        <div class="metric-card" style="padding: 1.5rem 1rem; text-align: center; border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                            <div class="metric-value" style="font-size: 1.8rem; margin: 0.5rem 0;">{s2_complete}</div>
                            <div class="metric-label" style="font-size: 0.8rem;">{s2_complete/s2_total*100:.0f}% Fully Covered</div>
                        </div>
                    </div>
                    <div class="metrics-grid" style="margin-top: 0.5rem;">
                        <div class="metric-card" style="padding: 1.5rem 1rem; text-align: center; border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                            <div class="metric-value" style="font-size: 1.8rem; margin: 0.5rem 0;">{s2_partial}</div>
                            <div class="metric-label" style="font-size: 0.8rem;">{s2_partial/s2_total*100:.0f}% Partially Covered</div>
                        </div>
                        <div class="metric-card" style="padding: 1.5rem 1rem; text-align: center; border-top: 3px solid #555; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                            <div class="metric-value" style="font-size: 1.8rem; margin: 0.5rem 0;">{s2_missing}</div>
                            <div class="metric-label" style="font-size: 0.8rem;">{s2_missing/s2_total*100:.0f}% Not Covered</div>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                """, unsafe_allow_html=True)
                st.info("No IFRS S2 requirements in current analysis")
        ...
        # Model Logging section
        st.markdown("---")
        st.markdown("### ➤ Model Logging")
        
        # Sample model logging JSON data
        model_logging_data = {
            "session_id": f"session_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            "timestamp": datetime.now().isoformat(),
            "model_version": "gap-analysis-v2.1.0",
            "processing_details": {
                "file_name": st.session_state.file_analysis_data['name'] if st.session_state.file_analysis_data else "sample_report.pdf",
                "pages_processed": st.session_state.file_analysis_data['pages'] if st.session_state.file_analysis_data else 45,
                "materiality_topics_extracted": len(st.session_state.materiality_topics),
            ...
            "analysis_results": {
                "total_coverage": {
                    "score": "55%",
                    "fully_covered": total_complete,
                    "partially_covered": total_partial,
                    "not_covered": total_missing
                },
                "ifrs_s1_coverage": {
                    "score": "55%",
                    "requirements_count": len(s1_df) if len(s1_df) > 0 else 4,
                    "fully_covered": s1_complete if len(s1_df) > 0 else 1,
                    "partially_covered": s1_partial if len(s1_df) > 0 else 1,
                    "not_covered": s1_missing if len(s1_df) > 0 else 2
                },
                "ifrs_s2_coverage": {
                    "score": "56%",
                    "requirements_count": len(s2_df) if len(s2_df) > 0 else 4,
                    "fully_covered": s2_complete if len(s2_df) > 0 else 1,
                    "partially_covered": s2_partial if len(s2_df) > 0 else 2,
                    "not_covered": s2_missing if len(s2_df) > 0 else 1
                }
            },
            "topics_identified": [topic['topic'] for topic in st.session_state.materiality_topics],
            "effort_assessment": "Medium"
        }
        
        # Display JSON in a scrollable code block with reduced height
        st.markdown("""
        <div style="height: 400px; overflow-y: scroll; border: 1px solid #ddd; border-radius: 8px; padding: 1rem; background-color: #f8f9fa;">
            <pre style="margin: 0; font-family: 'Courier New', monospace; font-size: 0.85rem; line-height: 1.4;">
                <code>{}</code>
            </pre>
        </div>
        """.format(json.dumps(model_logging_data, indent=2)), unsafe_allow_html=True)
        st.markdown("  ")
        # Download button for JSON
        json_str = json.dumps(model_logging_data, indent=2)
        col1, col2, col3, col4, col5, col6 = st.columns(6)
        ...
        with col1:
            st.download_button(
                label="Download JSON",
                data=json_str,
                file_name=f"model_logging_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
